...
window.COA = (function () {
  ...
  const generate = function (parent, kinship, dominion, type) {
    ...
    const addCharge = P(usedPattern ? 0.5 : 0.93) ? true : false; // 80% for charge
    const linedOrdinary =
    ...
    const ordinary = (!addCharge && P(0.65)) || P(0.3) ? (linedOrdinary ? linedOrdinary : rw(ordinaries.straight)) : null; // 36% for ordinary
    ...
    if (addCharge) {
      ...
      if (chargeData.colors > 1) item.t2 = P(0.25) ? getTincture("charge", usedTinctures, coa.t1) : t;
      if (chargeData.colors > 2) item.t3 = P(0.5) ? getTincture("charge", usedTinctures, coa.t1) : t;
      coa.charges = [item];
